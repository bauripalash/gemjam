...
impl StructureBuilder {
    ...
    pub fn get_post_title(&mut self, p: PathBuf , file_name : &String) -> Option<String> {
        ...
        let output_path  = self.output_dir.join(&self.slug).join(file_name.to_owned() + &".gmi".to_string());
        //if output_path.exists(){
            
        //    std::fs::remove_file(&output_path).expect("An outdated file exists in your output directory, and sadly I failed to delete it. So.... bye!");

        //}
        let mut fdsc = File::create(&output_path).expect("Failed to create a file for your post in output directory!");
        fdsc.write(_out_post.as_bytes()).expect("Failed to save post!");        //fs::write(output_path, &*_out_post).unwrap();
        output 
    }
    ...
    pub fn get_posts(&mut self) -> Vec<GemPost> {
        ...
        for _posts in self.content_dir.read_dir().unwrap() {
            ...
            gemposts.push(GemPost {
                ...
                title: self.get_post_title(_posts.as_ref().unwrap().path().to_path_buf() , &_fn_vec[0]),
        ...
        if index_path.exists() {
            ...
            for i in x {
                ...
                if self.reptags.contains_key(&_r) {
                    ...
                    println!("{}" , _filename);
                    _replace_with =
                        fs::read_to_string(self.template_dir.join(_filename))
            ...
            let mut index_file_dsc = File::create(out_index_path).expect("Cannot create index file in output directory");
            index_file_dsc.write(_raw.as_bytes()).expect("Failed to save index file in output directory");

        } else {
            println!("[Err] => Can not find the entry file {} at the templates folder of your theme!" , self.index_entry.file_name().unwrap().to_str().unwrap());
            no_error = false;
        }
